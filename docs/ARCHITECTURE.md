...
4. **运维验证层**  
   `scripts/context_healthcheck.sh`、`scripts/context_smoke.py`、`scripts/smoke_installed_runtime.py` 以及 `benchmarks/run.py` 统一保障 local-first 路线的安装态可用性和性能。Smoke 脚本依次调用 `context_cli health`/`quality gate`/`semantic`/`serve`，benchmark harness 驱动 `context_cli health`/`search`、`session_index.sync` 基准。
