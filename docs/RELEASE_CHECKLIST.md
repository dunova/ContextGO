...
- [ ] 安装态 `INSTALL_ROOT=~/.local/share/context-mesh-foundry/scripts` 至少包含 `context_cli.py` 与 `e2e_quality_gate.py`，并同时保留 `context_healthcheck.sh`、`benchmarks/run.py` 等运维入口，确保 smoke、healthcheck 与 benchmark 都能在安装态复现。
...
- [ ] 验证发布包在 `~/.local/share/context-mesh-foundry/scripts`（`scripts/smoke_installed_runtime.py` 的默认 `INSTALL_ROOT`）下包含 `context_cli.py`、`e2e_quality_gate.py`，以及 `context_healthcheck.sh`、`benchmarks/run.py` 等运维入口，并确保相关路径可读。
