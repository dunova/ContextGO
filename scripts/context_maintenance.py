...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Maintain ContextGO backlog and coverage")
    ...
    parser.add_argument("--include-subagents", action="store_true", help="Include subagent jsonl files")
    ...
    parser.add_argument("--stale-minutes", type=int, default=15, help="Treat processing jobs older than this as stale")
...
def enqueue_missing(
    ...
    for stype, path, sid in missing[: max(0, max_enqueue)]:
        ...
        existing = cur.execute("SELECT id, status FROM jobs WHERE dedupe_key=? LIMIT 1", (dedupe_key,)).fetchone()
        if existing:
