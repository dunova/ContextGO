...
- **Release pipeline can actually publish**: `pypa/gh-action-pypi-publish` was pinned to v1.13.0, whose bundled tooling rejects the `Metadata-Version: 2.5` produced by current hatchling, so every tag failed at `Publish to PyPI` with `InvalidDistribution`. The action is bumped to v1.14.2. / 发布流程此前因动作版本过旧拒绝新元数据而必然失败，已升级发布动作。
- **Test suite no longer mutates the repository it runs in**: `setup`/`unsetup` are project-scoped by design, and the suite exercised them with `cwd` set to the repo root — so running pytest rewrote (and, once policy-only files are removed, deleted) the checkout's own `.cursorrules` and `.github/copilot-instructions.md`. Every test now runs in its own temporary working directory, two tests that patched the wrong module twin were fixed, and `_remove_scf_policy` round-trips a file byte-for-byte. / 测试套件不再改动自身仓库：所有用例在独立临时工作目录运行，修正两处打桩目标错位，策略移除保证逐字节还原。
