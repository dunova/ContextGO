...
额外注意：`python3 scripts/smoke_installed_runtime.py` 默认从 `INSTALL_ROOT=~/.local/share/contextmesh/scripts` 载入 `context_cli.py` 与 `e2e_quality_gate.py`；若本机仍残留旧安装目录，脚本会回退探测 `~/.local/share/context-mesh-foundry/scripts`。发布产物还应在默认 tree 下保留 `context_healthcheck.sh`、`benchmarks/run.py` 等运维入口，并确认 smoke 启动前路径可读。
