...
额外注意：`python3 scripts/smoke_installed_runtime.py` 默认从 `INSTALL_ROOT=~/.local/share/context-mesh-foundry/scripts` 载入 `context_cli.py` 与 `e2e_quality_gate.py`；若显式设置了 `CONTEXT_MESH_INSTALL_ROOT`/`CMF_INSTALL_ROOT`，脚本也会跟随该目录探测。发布产物还应在默认 tree 下保留 `context_healthcheck.sh`、`benchmarks/run.py` 等运维入口，并确认 smoke 启动前路径可读。
